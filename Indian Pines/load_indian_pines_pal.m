function [all_data, all_labels, label_name] = load_indian_pines_pal(classes,sample_num,file)
...
for i = 1:d
    class = classes(i);
    [idx1, idx2] = find(labels == class);
    if length(idx1) < sample_num(i)
        sample_num(i) = length(idx1);
        fprintf('Sample number for class %.0f reduced to %.0f\n',class,sample_num(i))
    end
    [~,smpl] = datasample(idx1,sample_num(i),'Replace',false);
    data = [];
    for j = 1:sample_num(i)
        data = [data reshape(indian_pines(idx1(smpl(j)),idx2(smpl(j)),:),[dim,1])];
    end
